...
describe('slicePhasesAt', () => {
  ...
  it('the head has turned away before the fibres finish growing', () => {
    // by the time strands are well grown you should be seeing the back of the head
    const atGrowth = slicePhasesAt(0.46)
    expect(atGrowth.turn).toBeGreaterThan(0.95)
    expect(atGrowth.filament).toBeLessThan(atGrowth.turn)
  })

  it('tilt, turn, dissolve, filament and settle are monotonically non-decreasing', () => {
    const mono = ['tilt', 'turn', 'dissolve', 'filament', 'settle'] as const
